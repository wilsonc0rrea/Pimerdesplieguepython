<template>
  <div class="form-wrapper">
    <section class="form-section">
      <div>
        <h2 class="text-2xl font-bold text-primary">Estadísticas de trabajos realizados</h2>
      </div>
      ...
      >
        <div v-if="personaSeleccionada">
          ...
          <p class="text-sm text-600 mb-4">
            Trabajos atendidos en fecha {{ fecha }}: con un total de {{ personaSeleccionada.servicios.length }}
          </p>
          ...
          >
            ...
            <Column field="usuario" header="Realizó" sortable/>
...
async function loadDataservicios() {
  try {
    const response = await fetch(URI_ROOT + `/api/cargarestadisticatrabajos/?mes=${fecha.value}`)
