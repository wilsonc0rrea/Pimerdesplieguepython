<template>
  <div class="form-wrapper">
    <h2 class="form-title">CREACIÓN DE INGRESOS</h2>
    <InputText
      v-model="Idingreso"
      type="text"
      readonly
    ...
    <section class="form-section">
      ...
      <h2 class="form-title">LISTADO DE INGRESOS</h2>
      ...
      <DataTable
        ...
        :sortOrder="-1"
      >
        <Column field="ingreso_id" header="#" style="width: 60px"></Column>
          ...
          <template #body="slotProps">
            <Button
              ...
              @click="() => {
                 Swal.fire({
                  title: 'Confirmación',
                  text: `Sr. usuario, está seguro que desea editar el ingreso: ${slotProps.data.concepto} con ID: ${slotProps.data.ingreso_id}`,
                  icon: 'info',
                  ...
                  if (result.isConfirmed) {
                    ...
                    Idingreso = slotProps.data.ingreso_id
            ...
            <Button
              ...
              @click="() => {
                 Swal.fire({
                  ...
                  text: `Sr. usuario, va eliminar el ingreso: ${slotProps.data.concepto} con el ID: ${slotProps.data.ingreso_id}`,
                  icon: 'success',
                  showCancelButton: true,
                  confirmButtonColor: '#374151',
                  cancelButtonColor: '#ef4444',
...
async function loadTableingresos() {
  try {
    const response = await fetch(URI_ROOT + '/api/cargartableingresos');
...
async function editarIngresos() {
  try {
    ...
    const response = await fetch(`${URI_ROOT}/api/updateingresos`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        concepto: concepto.value,
        cuentadestino_id: Idcuentadestino.value,
        ingreso_id: Idingreso.value,
        valor: valor.value,
...
async function deleteIngresos() {
  try {

    const response = await fetch(`${URI_ROOT}/api/deleteingresos/` + Idingreso.value, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
    })
...
const handleSubmit = async () => {
  ...
  loading.value = true
  try {
    const response = await fetch(`${URI_ROOT}/api/creaingresos`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        concepto: concepto.value,
